...
public class UserServiceImpl implements UserService{
	...
	@Override
	public List<DoctorResponseDto> getAllDoc() {

		List<Doctor> allWorkingDoctors = doctorRepo.findAllWorkingDoctors();

		// Debugging step: Check if doctors are fetched from DB
		if (allWorkingDoctors.isEmpty()) {
			System.out.println("No working doctors found in the database.");
		} else {
			System.out.println("Doctors fetched from DB: " + allWorkingDoctors);
		}

		List<DoctorResponseDto> doctorResponseDtos = new ArrayList<>();

		for (Doctor doctor : allWorkingDoctors) {
